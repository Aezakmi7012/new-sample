...
class ResultsDisplay:
    ...
    def show_reranker(
        ...
        cross_encoder : HuggingFaceCrossEncoder
            Cross-encoder used to re-score the reranked documents.
        top_n : int
        ...
        scores = cross_encoder.score(pairs)
        ranked = sorted(
            zip(scores, reranked_docs, strict=False),
            key=lambda x: x[0],
            reverse=True,
        )
    ...
    def compare(
        ...
        compression_retriever : ContextualCompressionRetriever
            Reranking retriever.
        cross_encoder : HuggingFaceCrossEncoder
            Cross-encoder used for scoring in both display modes.
        top_k : int
        ...
        self.show_reranker(query, compression_retriever, cross_encoder, top_n)
